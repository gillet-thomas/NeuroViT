...
def main():
    ...
    if not config["inference"] and not config["sweep"]:
        ...
        wandb.init(project="fMRI2Vec_Sweep", mode='online' if config["wandb_enabled"] else 'disabled', config=config, name=config["name"]) 
        ...
        wandb.agent(sweep_id, function=train_sweep, count=1)            # Start the sweep agent
